#include "testing.hpp"
void model_eigen_test (vector<Mat> images, vector<int> label, vector<int>& actual_label, vector<int>& predicted_label)
{
	...
	while (q < images.size())
	{
		...
		//creates the model
		Ptr<face::FaceRecognizer> model = face::createEigenFaceRecognizer(0, conf);
